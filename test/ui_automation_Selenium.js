...
// node server.js
// ./node_modules/.bin/mocha --timeout 30000 test/ui_automation_Selenium.js
...
describe('NewsWatcher UI exercising', function () {
  ...
  before(function (done) {
    ...
    driver = new webdriver.Builder().withCapabilities(webdriver.Capabilities.ie()).build();
    ...
    driver.wait(webdriver.until.elementLocated(webdriver.By.id('loginLink')), 10000).then(function (item) {
      done();
    });
  });
  ...
  it('should deny a login with a non-registered email', function (done) {
    ...
    driver.findElement(webdriver.By.id('formControlsEmail2')).sendKeys('T@b.com');
    driver.findElement(webdriver.By.id('formControlsPassword2')).sendKeys('abc123*');
    driver.findElement(webdriver.By.id('btnLogin')).click();
    driver.wait(webdriver.until.elementLocated(webdriver.By.id('currentMsgId')), 5000);
    // It takes a second or two for the UI message to update, so we have to put in a timeout
    setTimeout(function () {
      driver.findElement(webdriver.By.id('currentMsgId')).getText().then(function (value) {
